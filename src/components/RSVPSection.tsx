...
export default function RSVPSection({ onValidated, onProceed }: RSVPSectionProps) {
    ...
    const handleCheck = () => {
        ...
        const potentialMatches = GUESTS.filter(g =>
            (g.husbandKeys && g.husbandKeys.some(k => input.includes(k) || k.includes(input))) ||
            (g.wifeKeys && g.wifeKeys.some(k => input.includes(k) || k.includes(input)))
        );
        ...
        setTimeout(() => {
            ...
            const bestMatch = potentialMatches.find(g => {
                const hasHusband = g.husbandKeys ? g.husbandKeys.some(k => input.includes(k)) : false;
                const hasWife = g.wifeKeys ? g.wifeKeys.some(k => input.includes(k)) : false;
            ...
            const inputHasHusband = activeMatch.husbandKeys ? activeMatch.husbandKeys.some(k => input.includes(k)) : true;
            const inputHasWife = activeMatch.wifeKeys ? activeMatch.wifeKeys.some(k => input.includes(k)) : true;
            const inputHasChild = activeMatch.childKeys ? activeMatch.childKeys.some(k => input.includes(k)) : true;

            const missing = [];
            if (activeMatch.husband && !inputHasHusband) missing.push(activeMatch.husband);
            if (activeMatch.wife && !inputHasWife) missing.push(activeMatch.wife);
